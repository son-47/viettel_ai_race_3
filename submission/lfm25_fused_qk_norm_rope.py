...
LFM2.5-1.2B has six attention layers.  The stock vLLM path materializes
contiguous Q and K tensors, launches RMSNorm separately for each, and then
launches RoPE.  This kernel reads the strided Q/K views straight from the
packed QKV projection and writes the final contiguous attention inputs in one
launch.  It follows the rounding boundary used by vLLM's upstream Qwen3.5
fusion: RMSNorm is rounded to the input dtype before RoPE arithmetic.
"""
...
def _lfm25_fused_qk_norm_rope_kernel(
    ...
    # RMSNorm variance covers the complete head.  LFM2.5 uses head_dim=64.
    ...
    # Support a pass-through RMSNorm tail for safety, although the target LFM
    # configuration uses full-head rotary_dim == head_dim.
    if HAS_PASS:
        weight = tl.load(
    ...
    # NeoX RoPE rotates the two contiguous halves of the rotary prefix.
    ...
    # Preserve stock memory semantics: RMSNorm writes BF16/FP16 before RoPE.
...
def fused_lfm25_qk_rmsnorm_rope(
    ...
    """Return contiguous post-RMSNorm, post-RoPE Q and K tensors."""
