"""GPU correctness and latency smoke test for the fused LFM2.5 kernel."""
...
def run_case(batch: int, state_dtype: torch.dtype, iterations: int) -> dict:
    ...
    b = torch.randn(
        batch, dim, device=device, dtype=torch.bfloat16, generator=generator
    )
    ...
    if batch > 1:
        # Exercise CUDA-graph padding semantics without touching state[-1].
        indices[-1, 0] = NULL_BLOCK_ID

    reference_state = state.clone()
    fused_state = state.clone()
    reference_output = stock_step(
    ...
    stock_us = elapsed_us(
        lambda: stock_step(b, c, x, stock_state, weight, indices),
        iterations,
    )
    fused_us = elapsed_us(
...
def main() -> None:
    ...
    for state_dtype in (torch.bfloat16, torch.float32):
        for batch in args.batches:
            results.append(run_case(batch, state_dtype, args.iterations))
