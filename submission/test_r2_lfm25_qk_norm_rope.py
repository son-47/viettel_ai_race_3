"""GPU parity and latency smoke test for the R2 LFM2.5 QK fusion."""
...
def run_case(token_count: int, dtype: torch.dtype, iterations: int) -> dict:
    ...
    eps = 1e-5
    q_weight = torch.randn(HEAD_DIM, device=device, dtype=dtype, generator=generator)
    k_weight = torch.randn(HEAD_DIM, device=device, dtype=dtype, generator=generator)
...
def main() -> None:
    ...
    results = [
        run_case(token_count, dtype, args.iterations)
